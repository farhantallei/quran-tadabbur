...
const Chapters = ({ chapters, currentId, setCurrentId, setTitle }) => {
    ...
    const openChapter = () => {
        history.push(`/surah/${chapters.chapter_id}`)
        setTitle(`Surah no ${chapters.chapter_index}`)
    }

    return (
        <div className={currentId === chapters._id ? 'card card-active' : 'card'} onDoubleClick={openChapter}>
            <div className='action'>
                <div className='index'>{chapters.chapter_index}</div>
